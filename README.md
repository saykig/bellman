...
The [anytime-valid Bernoulli data-to-decision bridge](foundations/BELLMAN_ANYTIME_VALID_DATA_TO_DECISION.md) constructs an outward-checked rational interval from a complete binary prefix under an explicit IID fixed-parameter premise, then separately checks exact static action-risk and named-action regret over that interval. Its simultaneous coverage survives repeated inspection and data-dependent stopping under the stated sampling assumptions; it is not a posterior, empirical validation, sequential-control theorem, or Writ/Decision Lab integration. See [verification](verification/statistical_decision_bridge/README.md).
